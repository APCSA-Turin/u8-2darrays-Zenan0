...
public class ItemGrid {
    ...
    public String mostValuableNeighbor(int r, int c) {
        int highestValue = grid[r][c].getValue();
        String highestValueItem = grid[r][c].getName();
        if (isValid(r, c - 1)) {
          if (grid[r][c - 1].getValue() > highestValue) {
            highestValue = grid[r][c - 1].getValue();
            highestValueItem = grid[r][c - 1].getName();
          }
        }
        if (isValid(r, c + 1)) {
          if (grid[r][c + 1].getValue() > highestValue) {
